...
###current colors: flatly theme -turquoise compound


##colors as a vector

cols=c("#0AB5F2", "#304E59", "#1BBF91","#C7574E", "#F20A40")

cols3=c("#0AB5F2", "#F20A40", "#304E59")

####MAKE MY HC THEME###

  thm <- hc_theme(
  colors = c("#304E59","#0AB5F2",  "#1BBF91","#C7574E", "#F20A40"),
  chart = list(
    backgroundColor = ""
  ),
  title = list(
...
leaflet(data=bsc)%>%
  ...
  addPolygons(data=bsc, color="#1BBF91", weight=1.5, opacity = 0, fillOpacity = .6,
...
hc<-race_clb%>%
  hchart(type = "treemap", hcaes(x = variable, value = value, color=value),
   tooltip = list(pointFormat = "{point.variable} Percentage: {point.value:,.1f}%"))%>%
      hc_tooltip(crosshairs = TRUE)%>%
  hc_colorAxis(stops = color_stops(n=5, colors = c("#087355", "#3DFFC8", "#1BBF91","#731A00", "#BF401B")))%>%
     hc_title(
...
hc<-race_lac%>%
  hchart(type = "treemap", hcaes(x = variable, value =value, color = value),
         tooltip = list(pointFormat = "{point.variable} Percentage: {point.value:,.1f}%"))%>%
      hc_tooltip(crosshairs = TRUE)%>%
    hc_colorAxis(stops = color_stops(colors = c("#087355", "#3DFFC8", "#1BBF91","#731A00", "#BF401B")))%>%
   hc_title(
...
hc<-uninsured_clb %>%
  ...
  # hc_colorAxis(minColor="#BF401B", maxColor = "#1BBF91")%>%
      hc_legend(enabled = FALSE)%>%
  hc_title(
...
leaflet(height=1000, width=1000) %>%
  ...
  addPolygons(data=bsc, color="#1BBF91", weight=2, opacity = 1, fillOpacity =.3, group="Existing BSCs") %>%
...
hc<-la_rates %>%
  ...
  hchart('column', hcaes(x = 'Race', y = 'Percent', group = group),  tooltip = list(pointFormat = "Percentage: {point.value:,.1f}%"))%>%
  hc_title(
...
kbl(rate_table, align="c", col.names = c("", "Maltreated Children", "Children Under 18", "Maltreatment Rate"))%>%
