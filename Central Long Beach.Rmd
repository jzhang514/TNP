...
leaflet(data=bsc)%>%
  ...
  addPolygons(data=bsc, color="#79D9B6", weight=3, opacity = 1, fillOpacity = .2,
...
children<-dbGetQuery(con, "SELECT * FROM ct_youngchildren_2019")
...
race<-dbGetQuery(con, "SELECT * FROM ct_raceethnicity_2019")
...
valueBox("0 Open and 2 Closed", "Licensed Child Care Centers or Family Child Care Homes in Central Long Beach",
         href="https://www.ccld.dss.ca.gov/carefacilitysearch/DownloadData",
         color="#79D9B6")
```
...
#view graph
 ...
 ece_it_lac<-dbGetQuery(con, "SELECT * FROM zip_unmet_ece_it_2018")%>%
  gather(measure, value, children_it:pct_unmet_need_children_it)%>%
  filter(!grepl("pct", measure))%>%
  filter(!grepl("unmet_need_children_it", measure))%>%
  mutate(variable=ifelse(measure %in% 'children_it', 'Total Children',
...
#get order correct for stacking
# ece_it_lac <- within(ece_it_lac, variable <- factor(variable,
#                                            levels=c("Total Children","Total Enrolled", "Total Eligible")))
# 
# 
#  ggplot(ece_it, aes(fill=variable, y=value, x=best_start)) + 
#     geom_bar(position="stack", stat="identity")+
#     xlab("")+
#   ylab("")+
#   labs(title="Percentage of Students Prepared for College by Race")+
...
hc<-force%>%
  ...
  hc_title(
    text = "A Black individual is 6 times more likely to be injured in law enforcement incidents compared to a White indivdiual",
    margin = 20,
    align = "left",
    style = list(useHTML = TRUE, fontWeight="bold")
    )%>%
